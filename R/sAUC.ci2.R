...
#' (ci <- sAUC.ci(opt1, B = 5))
#'
sAUC.ci <- function(object, B = 10, ncores, type = "SOCK",
                    ci.level = 0.95,
                    hide.progress = FALSE)
{
  ...
  bd <- sapply(1:6, function(i){

    q1 <- quantile(scale(PAR[i, ]), probs = (1-ci.level)/2)
    q2 <- quantile(scale(PAR[i, ]), probs = 1-(1-ci.level)/2)
    se <- sd(PAR[i, ])
    c(q1*se, q2*se)
  } )
  ...
  par.h <- object$par[c(1:5, 11)]
  CI <- matrix(c(par.h, par.h), nrow = 6) + t(bd)
  rownames(CI) <- c("u1", "u2", "t1", "t2", "r", "auc")

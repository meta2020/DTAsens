##
## OPTIMIZATION FUNCTION 1; WITH c1^2 c2^2 FIXED
##
## CREATE: 2020-12-22: CHANGE NAME AND REVISE THE DETAILS
## REVISE: 2021-01-04: CHECK
##
## INPUT:  data, p, c(c1, c2), 
##         START POINT 6 PARS: c(u1, u2, t1, t2, r, b)
##         OPTIONAL: "b.width"
##         OTHERS
##
## OUTPUT: "u1", "u2", "t11", "t22", "t12", "r", "auc", "b", "a" ("p.hat"), "conv", 
##        ("data")
## DEPEND: correction function, DOR.data function
...
optim1 <- function(data,   ## 2 FORMAT: N OR Y, make data name as format
                  ...
                  ## STARTING EST POINTS
                  start6 = c(0, 0, 1, 1, -0.5, 0),  ## u1, u2, t1, t2, r, b
  ...
  fn <- function(par) {
    ...
    if(is.null(start6)) {
      ...
      if(!inherits(fit.m, "try-error")) {
        ...
        p2 <- sqrt(fit.m$Psi[4])
        p12<- fit.m$Psi[3]
        start6 <- c(fit.m$coefficients, p1,p2, p12/(p1*p2), 0)
        
      } else start6 <- c(0,0,1,1,-0.5,0,0.7)
  ...
  if(!class(opt)=="try-error") {
    ...
    opt$par <- c(u1, u2, t11, t22, t12, r, auc, b, a.opt)

    names(opt$par) <- c("u1", "u2", "t11", "t22", "t12", "r", "auc", "b", "a")
    ...
    if (p.hat){
      ...
      opt$par   <- c(u1, u2, t11, t22, t12, r, auc, b, a.opt, p.hat)

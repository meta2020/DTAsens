...
mAUC <- function(par.matrix){
  ...
  sapply(1:ncol(par.matrix), function(i) {
    ...
    if (NA %in% par.matrix[,i]) {auc <- NA} else {
      ...
      auc.try   <- try(integrate(function(x) { plogis(u1 - (r*t1/t2) * (qlogis(x) + u2)) }, 0, 1), silent = TRUE)
      #auc.l.try <- try(integrate(function(x) { plogis(u1 - (r*t1/t2) * (qlogis(x) + u2) - qnorm((1-ci.level)/2, lower.tail = FALSE) * t1* sqrt(1-r^2)) }, 0, 1), silent = TRUE)
      #auc.u.try <- try(integrate(function(x) { plogis(u1 - (r*t1/t2) * (qlogis(x) + u2) + qnorm((1-ci.level)/2, lower.tail = FALSE) * t1* sqrt(1-r^2)) }, 0, 1), silent = TRUE)

      if(!class(auc.try)=="try-error")   auc.try$value   else NA
      #if(!class(auc.l.try)=="try-error") auc.l <- auc.l.try$value else auc.l <- NA
      #if(!class(auc.u.try)=="try-error") auc.u <- auc.u.try$value else auc.u <- NA
...
mAUC.ci <- function(par.matrix, ci.level = 0.95){
  ...
  vapply(1:ncol(par.matrix), function(i) {
    ...
    if (NA %in% par.matrix[,i]) {auc <- NA} else {
      ...
      if(!class(auc.try)=="try-error")   auc   <- auc.try$value   else auc <- NA
      if(!class(auc.l.try)=="try-error") auc.l <- auc.l.try$value else auc.l <- NA
      if(!class(auc.u.try)=="try-error") auc.u <- auc.u.try$value else auc.u <- NA

      cbind(auc.l, auc, auc.u)

...
#' @examples
#'
#' opt1 <- dtasens1(IVD, p = 0.7, pos.r = TRUE)
#' (ci <- sAUC.ci(opt1, B = 5))
#'
                    ...
                    hide.progress = FALSE)
{
  ...
  s.sauc.t <- scale(sauc.t)
  q1 <- quantile(s.sauc.t, probs = (1-ci.level)/2)
  q2 <- quantile(s.sauc.t, probs = 1-(1-ci.level)/2)
  ...
  if(ROC.plot){
    ...
    msROC(PAR)
    sROC(object, add = TRUE, roc.col = 2)
    sROC(PAR[,q2.pos], add = TRUE)
